...
Log processor service connects to the Loggerator service to retrieve user access logs.
Access logs can be filtered by Http response codes, request methods, and usernames and are returned in decending order.

Swagger API documentation can be found here and includes an ability to invoke this end-point method and review results.
http://localhost:8081/swagger-ui/#/log-controller

Requirements (local dev):
1.) Loggerator must be running on port 8080. 
Loggerator is initiated through Docker. Parameters "count" and "seed" are optional. Additional details below.   
docker run -p 8080:8080 gcr.io/hiring-278615/loggerator --count 1500 --seed 300
2.) Run log-processor-service through development application (such as IntelliJ) 
  or "mvn clean package" and start from the generated jar.
Note: "dev" profile must be specified.

To run and test as a Docker container:
1.) Run docker-compose.yml
This will build the Docker images and run both the Loggerator and the Log Processor Service application.
